...
const ContactPage = () => {
  ...
  const performPost = (operation, object) => {
    ...
    axios
      .post(`${baseUrl}/${operation}Contact${plural}.php`, object)
      .then((response) => {
  ...
  const deleteContact = (contact) =>
    performPost("Delete", { id: contact.id, userId: currentUser.id });
...
const HeaderBar = ({ currentUser }) => {
  ...
  return (
    <div>
      <nav
        id="AuthPage-nav"
        className="row align-items-center grid"
      >
        <div class="col container left-align">
          <img
        ...
        <div className="col">
          <span id="header-right-span">
            ...
            <button className="log-out button-4" onClick={logOut}>
...
const ContactBook = ({
  ...
  const contactContainsSearch = (contact) => {
    return `${contact.firstName.toLowerCase()} ${contact.lastName.toLowerCase()} ${
      contact.phoneNumber
    } ${contact.emailAddress}`.includes(search);
  ...
  return (
    <main>
      <span id="contact-book-header">
        ...
        />
        <button onClick={handleAdd} className="button-4">
          Add contact
        </button>
      </span>
      <table>
        ...
        <tbody>
          {isAdding ? (
            <AddContact addContact={addContact} setIsAdding={setIsAdding} />
          ) : null}
          {tableRows}
        </tbody>
      </table>
    ...
    <tr>
      ...
      <td>
        <span className="action-menu">
          <input form={-1} type="submit" className="button-4" />
          <button className="button-4" onClick={() => setIsAdding(false)}>
            Cancel
          </button>
        </span>
      </td>
...
const Contact = ({ contact, updateContact, deleteContact }) => {
  ...
  const actionMenu = !isDeleting ? (
    <span className="action-menu">
      <button onClick={() => setIsEditing(true)} className="button-4">
        Edit
      </button>
      <button onClick={() => setIsDeleting(true)} className="button-4">
        Delete
      </button>
    </span>
  ) : (
    <span className="action-menu">
      <button onClick={() => setIsDeleting(false)} className="button-4">
        Cancel
      </button>
      <button
        onClick={() => {
  ...
  const editableContact = (
    <tr>
      ...
      <td>
        <span className="action-menu">
          <input form={contact.id} type="submit" className="button-4" />
          <button className="button-4" onClick={() => setIsEditing(false)}>
            Cancel
          </button>
        </span>
      </td>
  ...
  return !isEditing ? (
    <tr>
      ...
      <td className="action-menu">{actionMenu}</td>
    </tr>
  ) : (
    editableContact
  );
